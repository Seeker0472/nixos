...
playerName=$(playerctl --list-all | grep Netease) || exit 0
#playerName="feeluown"
...
song(){
  ...
  if [[ $title != "" ]]; then
    ...
    echo "$(sed -n 2p $lyricsPath | tr -d '\r')" 
  fi
}
...
notifySongInfo(){
  ...
  # 弹出提示框
  notify-send -h string:x-dunst-stack-tag:music "$title-$artist" "$album" -t 800 --icon "$songIcons/$songId.$iconSu" --category=waybarlrc
}
